...
while true; do 
  ...
  google-chrome --kiosk --no-first-run --incognito --no-default-browser-check --noerrdialogs --disable-session-crashed-bubble --disable-pinch --enable-precise-memory-info --max_old_space_size=4096 --disable-translate --disable-features=TranslateUI --simulate-outdated-no-au='Tue, 31 Dec 2099 23:59:59 GMT'  --autoplay-policy=no-user-gesture-required --disk-cache-dir=/tmp/cache --disable-gpu-vsync 'http://localhost'
